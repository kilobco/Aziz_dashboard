...
with tab_analytics:
    ...
    with st.container(border=True):
        ...
        daily_branch = (
            ...
            .sum()
            .reset_index()
            .rename(columns={'location_name': 'Branch', 'total_amount': 'Revenue (LBP)'})
        )
        ...
        cols[0].metric("Total Revenue (7d)", f"LBP {total_rev:,.0f}")
        for i, branch in enumerate(branches):
            branch_rev = txn_7[txn_7['location_name'] == branch]['total_amount'].sum()
            short = branch
            cols[i + 1].metric(f"{short}", f"LBP {branch_rev:,.0f}")

        fig_sales = px.line(
            daily_branch, x='date', y='Revenue (LBP)', color='Branch',
